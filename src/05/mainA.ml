...
let process_updates edges updates =
  ...
  List.iter
    (fun update ->
      ...
      | Some cycle ->
          Printf.printf "Update %s causes a cycle: %s\n"
            (String.concat "," (List.map string_of_int update))
            (String.concat " -> " (List.map string_of_int cycle))
      | None -> (
          match topological_sort graph with
          | Some topo_order ->
              if is_correct_order update topo_order then
                valid_updates := update :: !valid_updates
          | None -> ()))
    updates;

  (* Sum of middle pages *)
  List.fold_left (fun acc update -> acc + middle_page update) 0 !valid_updates
...
let () =
  ...
  Printf.printf "Sum of middle pages in correctly-ordered updates: %d\n" result
