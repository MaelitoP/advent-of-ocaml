...
let concat n1 n2 =
  let len = int_of_float (log10 (float_of_int n2)) + 1 in
  n1 * int_of_float (10. ** float_of_int len) + n2

(* Recursive function with pruning and memoization *)
let rec evaluate_recursive numbers target index current_result memo =
  if index = List.length numbers then current_result = target
  else if (* Check memoization table *)
          Hashtbl.mem memo (index, current_result)
  then Hashtbl.find memo (index, current_result)
  else
    ...
    if current_result + next_num <= target then
      valid :=
        !valid
        || evaluate_recursive numbers target (index + 1)
    ...
    if concat_result <= target then
      valid :=
        !valid
        || evaluate_recursive numbers target (index + 1) concat_result memo;
